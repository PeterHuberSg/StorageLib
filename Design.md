...
public class Share {
  ...
  public SortedList<Date, Quote> Quotes;
}
...
Sometimes it is helpful to group children by `Date`, but several children might have the same
`Date`. Example of storing share sales, where several sales can happen on the same date:

```csharp
public class Share {
  public string CompanyName;
  public SortedBucketCollection<Date, Sale> Sales;
}
...
`SortedBucketCollection<Key1, Child>` groups all children by *Key1*. Several children can have the 
same *Key1* value; within a bucket they are kept in the order they were added. The two key form 
`SortedBucketCollection<Key1, Key2, Child>` additionally sorts the children within a bucket by 
*Key2*, and the combination of *Key1* and *Key2* must be unique. The child's autogenerated *Key* 
cannot be used as *Key2*. This collection was specifically written for *StorageLib*.
