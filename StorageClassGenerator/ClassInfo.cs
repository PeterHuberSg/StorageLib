  ...
  public class ClassInfo {
      ...
      if (toLower!=null) {
        ...
        if (childKeyPropertyName is not null) {
          throw new GeneratorException($"{ClassName}.{name} is a lower case copy of property {toLower}. It cannot have a " +
            "StoragePropertyAttribute with a childKeyPropertyName argument, which is only for Directories and SortedLists:" + Environment.NewLine + memberText);
        }
        ...
        if (csvTypeString.Contains("<")) {
            ...
            csvTypeString.EndsWith(">")) 
          {
            ...
            //  Environment.NewLine + memberText);
            ////childKeyPropertyName = csvTypeString[(startCommentPos+2)..endCommentPos].Trim();
            //var stringWithoutComment = csvTypeString[..(startCommentPos)].Trim() + csvTypeString[(endCommentPos+2)..].Trim();
      ...
      foreach (var mi in Members.Values) {
        ...
        if (mi.MemberType==MemberTypeEnum.ParentMultipleChildrenList) {
          ...
          if (mi.IsNullable) {
            ...
            //**sw.WriteLine($"        if ({context}.Data._{mi.ParentClassInfo!.PluralName}.TryGetItem({mi.LowerMemberName}Key.Value, " +
            ...
            //**sw.WriteLine($"      if ({context}.Data._{mi.ParentClassInfo!.PluralName}." +
            //  $"TryGetItem({mi.ParentClassInfo!.LowerClassName}Key, out var {mi.LowerMemberName})) {{");
            //sw.WriteLine($"          {mi.MemberName} = {mi.LowerMemberName};");
            //sw.WriteLine($"      }} else {{");
    ...
    private void writeUpdate(StreamWriter sw, string context, List<string> lines, TracingEnum isTracing) {
      ...
      if (HasNotReadOnlyAndNotLookupParents) {
        sw.WriteLine();
        sw.WriteLine($"      //remove not yet updated item from parents which will be removed by update");
        foreach (var mi in Members.Values) {
          ...
          {
            string keyName = mi.ParentMemberInfo.ChildKeyPropertyName!;
            var lowerKeyName = keyName[0..1].ToLower() + keyName[1..];
            sw.WriteLine($"      var has{mi.MemberName}Changed = {mi.MemberName}!={mi.LowerMemberName} || {keyName}!={lowerKeyName};");
          } else {
      ...
      ////add item with changed values to new parents
      //foreach (var mi in Members.Values) {
      //  if (mi.IsReadOnly || mi.MemberType!=MemberTypeEnum.LinkToParent) continue;
    ...
    private void writeTransactionSupport(StreamWriter sw, string context, TracingEnum isTracing) {
        ...
        foreach (var mi in Members.Values) {
          ...
          if (mi.MemberType==MemberTypeEnum.LinkToParent) {
              ...
              parentMI.MemberType==MemberTypeEnum.ParentMultipleChildrenSortedList) 
            {
              ...
              //List or parent with 1 child only
              sw.WriteLine($"      var has{mi.MemberName}Changed = oldItem.{mi.MemberName}!=item.{mi.MemberName};");
            }
            if (mi.IsNullable) {
      ...
      foreach (var mi in Members.Values) {
        if (mi.MemberType==MemberTypeEnum.ToLower || mi.MemberType>=MemberTypeEnum.ParentOneChild || mi.IsReadOnly) continue;
