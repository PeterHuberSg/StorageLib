...
namespace TestContext  {
  ...
  public partial class DictionaryParentR: IStorageItemGeneric<DictionaryParentR> {
    ...
    readonly StorageDictionary<DictionaryParentR, string, DictionaryChild> dictionaryChildren;
    ...
    public DictionaryParentR(string text, bool isStoring = true) {
      ...
      Text = text;
      dictionaryChildren = new StorageDictionary<DictionaryParentR, string, DictionaryChild>(this);
#if DEBUG
    ...
    internal void AddToDictionaryChildren(DictionaryChild dictionaryChild) {
      ...
      DC.Trace?.Invoke($"Add DictionaryChild {dictionaryChild.GetKeyOrHash()} to " +
        $"{this.GetKeyOrHash()} DictionaryParentR.DictionaryChildren");
#endif
    }
      ...
      foreach (var dictionaryChild in DictionaryChildren.Values) {
        if (dictionaryChild?.Key>=0) {
          throw new Exception($"Cannot release DictionaryParentR '{this}' " + Environment.NewLine + 
            $"because '{dictionaryChild}' in DictionaryParentR.DictionaryChildren is still stored.");
        }
      }
