...
- `SortedBucketCollection<Key1, Child>`
...
`Dictionary<Key, Child>` or `SortedList<Key, Child>` for the parent's children 
collection, but when a child is releasable, it uses a `StorageDictionary<Key, Child>` or 
`StorageSortedList<Key, Child>`. `StorageDictionary<>` and `StorageSortedList<>` 
...
A `SortedBucketCollection` groups children by a first key (Key1), like all items which were 
created on the same day. This is different from a `Dictionary` or `SortedList`, where no 2 
children can have the same key value. A `SortedBucketCollection` comes in two forms:

* `SortedBucketCollection<Key1, Child>` (one key): children are grouped by Key1. Several children 
can share the same Key1 value; within a bucket they are kept in the order they were added.
* `SortedBucketCollection<Key1, Key2, Child>` (two keys): children are grouped by Key1 and, within 
a bucket, sorted by Key2. Only the combination of Key1 and Key2 must be unique.

**Example (one key):** A simplistic financial application might have accounts, like Cash or 
Expenses, and AccountItems stating on which date a certain amount was booked for an account. 
Several AccountItems can share the same date and there is no natural second key to tell them 
apart, so the one key form is used:

```csharp
public class Account {
  public string Name;
  public SortedBucketCollection<Date, AccountItem> Items;
}
...
**Example (two keys):** When the child has a property which uniquely identifies it within a 
bucket, that property can be used as second key. Here every AccountItem has a BookingNumber, so 
the AccountItems of one date get sorted by their BookingNumber:
...
**Note:** The child's autogenerated `Key` property cannot be used as second key. A child which is 
not stored yet has `Key==-1` (NoKey). If such children were added to a bucket sorted by `Key`, they 
would all share the value -1, and once they get stored and receive their real key the bucket order 
would be wrong. StorageClassGenerator reports an error if the child's `Key` would be used as second 
key. If you want to group children by Key1 and allow several children per Key1 without a meaningful 
second key, use the one key form `SortedBucketCollection<Key1, Child>` instead.

Both forms allow to query a Key1 range like this:
...
SortedBucketCollection was written specifically for StorageLib to group children by a key when 
several children can share that key. When a child is not releasable, *StorageClassGenerator* uses 
a normal `SortedBucketCollection`, but when a child is releasable, it uses a 
`StorageSortedBucketCollection`. `StorageSortedBucketCollection` behaves like a 
`SortedBucketCollection`. When enumerating its items, **all** children are shown, stored and not 
stored ones. `StorageSortedBucketCollection.GetStoredItems()` enumerates over all stored children, 
but skips over not stored children. `StorageSortedBucketCollection.CountStoredItems` returns only 
the number of stored children.
